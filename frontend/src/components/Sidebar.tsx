...
import { 
  Home, Bot, Command, BarChart3, Shield, Zap, Music, Server, BookOpen, Settings, Menu, X 
} from 'lucide-react';
...
export function Sidebar() {
  ...
  return (
    <>
      ...
      {/* Сайдбар */}
      <div className={`w-64 border-r border-[rgb(var(--border))] bg-[rgb(var(--surface))] flex flex-col h-screen fixed lg:static transition-all duration-300 z-40
        ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        
        {/* Логотип */}
        <div className="p-6 border-b border-[rgb(var(--border))]">
          <Link href="/" className="flex items-center gap-3 group" onClick={() => setIsOpen(false)}>
            <div className="w-10 h-10 bg-gradient-to-br from-nova-400 to-cyan-500 rounded-2xl flex items-center justify-center shadow-lg">
          ...
          {navItems.map((item) => (
            ...
            >
              <item.icon className={`w-5 h-5 transition-colors ${
                isActive(item.href) ? 'text-nova-400' : 'group-hover:text-nova-400'
              }`} />
              <span>{item.label}</span>
              
              {isActive(item.href) && (
                <div className="ml-auto w-1.5 h-1.5 bg-nova-400 rounded-full" />
              )}
            </Link>
          ))}
        </nav>
