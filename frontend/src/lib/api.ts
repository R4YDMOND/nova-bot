...
export const api = {
  servers: {
    list: () => apiFetch<{ servers: { id: number; name: string }[]; total: number }>('/api/servers'),
    create: (data: { name: string; server_id: string; webhook_url?: string }) => {
      ...
      return apiFetch<{ status: string }>(`/api/servers?${params}`, { method: 'POST' });
    },
  },
  modules: {
