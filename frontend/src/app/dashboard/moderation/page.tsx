        ...
        {TABS.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors ${
              activeTab === tab.id ? 'bg-[rgb(var(--surface-2))] text-[rgb(var(--text))]' : 'text-[rgb(var(--text-secondary))] hover:text-[rgb(var(--text))] hover:bg-[rgb(var(--surface-2))]'
            }`}>{tab.label}</button>
        ))}
      </div>
      ...
      {activeTab === 'protection' && (
        <Card className="p-6">
          ...
          {([
            ...
            <div key={item.key} className={`flex justify-between items-center py-3 ${i < arr.length - 1 ? 'border-b border-[rgb(var(--border))]' : ''}`}>
              <div>
      ...
      {activeTab === 'auto' && (
        <Card className="p-6">
          ...
          {([
            ...
            <div key={item.toggleKey} className={`flex justify-between items-center py-3 ${i < arr.length - 1 ? 'border-b border-[rgb(var(--border))]' : ''}`}>
            ...
            <div className="grid grid-cols-3 gap-3 mb-4">
              {MOD_AVATARS.map(av => (
                <button key={av.id} onClick={() => update('modAvatarStyle', av.id)}
                  ...
                  <div className={`text-xs font-medium ${settings.modAvatarStyle === av.id ? av.color : 'text-[rgb(var(--text-secondary))]'}`}>{av.label}</div>
                </button>
              ))}
            </div>
            <div>
