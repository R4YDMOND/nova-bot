      ...
      <div className="flex justify-between items-start flex-wrap gap-4">
        <div>
          ...
          <p className="text-[rgb(var(--text-secondary))]">
            Управляйте подключёнными серверами
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="py-16 text-center">
            ...
            <p className="text-[rgb(var(--text-secondary))] mb-4">
              Не удалось загрузить серверы. Проверьте подключение к backend.
            </p>
          ...
          <div className="py-16 text-center">
            ...
            <p className="text-[rgb(var(--text-secondary))] mb-4">
              У вас пока нет подключённых серверов
            </p>
            <Button onClick={() => (window.location.href = "/dashboard/servers/add")}>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[rgb(var(--border))] text-left text-xs uppercase text-[rgb(var(--text-secondary))]">
                ...
                <th className="py-4 px-4">Участников</th>
            ...
            <tbody>
              {servers.map((s: Server) => (
                <tr
                  key={s.id}
                  className="border-b border-[rgb(var(--border))] hover:bg-[rgb(var(--surface-2))] transition-colors"
                >
                  <td className="py-4 px-4 font-medium flex items-center gap-3">
                    {s.icon ? (
                      <img
                        src={`https://cdn.discordapp.com/icons/${s.id}/${s.icon}.webp?size=32`}
                        alt={sanitize(s.name)}
                      ...
                      <div className="w-8 h-8 rounded-full bg-indigo-600 flex items-center justify-center text-xs font-bold">
                        {sanitize(s.name).charAt(0).toUpperCase()}
                      </div>
                    )}
                    {sanitize(s.name)}
                  </td>
                  <td className="py-4 px-4 text-[rgb(var(--text-secondary))] text-xs font-mono">
                    {s.id}
                  </td>
                  <td className="py-4 px-4 text-[rgb(var(--text-secondary))]">
                    {s.member_count?.toLocaleString("ru-RU") ?? "—"}
                  </td>
