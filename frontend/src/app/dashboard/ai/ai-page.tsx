...
function Toggle({ checked, onChange }: { checked: boolean; onChange: () => void }) {
  return (
    <button
      onClick={onChange}
      className={`relative w-11 h-6 rounded-full transition-colors ${checked ? "bg-indigo-500" : "bg-zinc-700"}`}
    >
      <span
...
export default function AIPage() {
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="flex justify-between items-start flex-wrap gap-4">
        <div>
          ...
          <p className="text-[rgb(var(--text-secondary))] text-sm mt-1">
            Гибкая настройка моделей и стилей общения 🤖
          </p>
        </div>
        <Button onClick={save}>
        ...
        {TABS.map((tab) => (
          <button
            ...
            className={`px-4 py-2 rounded-2xl text-sm font-medium transition-all ${
              activeTab === tab.id
                ? "bg-[rgb(var(--surface-2))] border border-[rgb(var(--border))] text-white"
                : "border border-transparent text-[rgb(var(--text-secondary))] hover:text-white hover:bg-[rgb(var(--surface-2))]"
            }`}
          >
            {tab.label}
          </button>
          ...
          <Card className="p-6">
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                <button
                  key={m.value}
                  onClick={() => update("activeModel", m.value as AISettings["activeModel"])}
      ...
      {activeTab === "gemini" && (
        <Card className="p-6 space-y-5">
          <div className="flex justify-between items-center">
            ...
            <Toggle checked={settings.geminiEnabled} onChange={() => toggle("geminiEnabled")} />
          </div>
          {settings.geminiEnabled && (
            <>
              ...
              <div>
                <h4 className="text-sm font-semibold text-[rgb(var(--text-secondary))] mb-3">🎨 Стиль общения</h4>
                <div className="space-y-2">
                  {GEMINI_STYLES.map((style) => (
                    <button
                      key={style.value}
                      onClick={() => update("geminiStyle", style.value)}
      ...
      {activeTab === "deepseek" && (
        <Card className="p-6 space-y-5">
          <div className="flex justify-between items-center">
            ...
            <Toggle checked={settings.deepseekEnabled} onChange={() => toggle("deepseekEnabled")} />
          </div>
          {settings.deepseekEnabled && (
            <>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {DEEPSEEK_STYLES.map((style) => (
                    <button
                      key={style.value}
                      onClick={() => update("deepseekStyle", style.value)}
                      className={`w-full p-4 rounded-2xl text-left border-2 transition-all ${
              ...
              <div>
                ...
                <textarea
                  value={settings.deepseekCustomPrompt}
          ...
          <Card className="p-6">
            ...
            <div className="grid grid-cols-4 gap-3 mb-4">
              {AVATARS.map((avatar) => (
                <button
                  key={avatar.id}
                  onClick={() => update("avatarStyle", avatar.id)}
                ...
                >
                  <div
                    className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl mx-auto mb-2 bg-[rgb(var(--surface))]"
      ...
      {/* Тост при сохранении */}
      {saved && (
        <div className="fixed bottom-6 right-6 bg-green-500 text-white px-5 py-3 rounded-2xl font-semibold shadow-lg z-50 animate-bounce">
