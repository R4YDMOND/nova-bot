    ...
    <div style={{ minHeight: '100vh', background: '#0A0A0F', display: 'flex', color: '#F1F5F9' }}>
      ...
      <main style={{ flex: 1, padding: '40px 48px', overflow: 'auto' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', marginBottom: '20px', flexWrap: 'wrap', alignItems: 'center' }}>
          <input type="text" value={sanitize(searchQuery)} onChange={(e) => setSearchQuery(e.target.value)}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {filtered.map((cmd, i) => (
                ...
                >
                  <td style={{ padding: '12px 16px' }}><code style={{ background: '#0A0A0F', padding: '4px 10px', borderRadius: '6px', color: '#00E5FF', fontSize: '13px', fontFamily: 'monospace' }}>{sanitize(cmd.name)}</code></td>
                  <td style={{ padding: '12px 16px', fontSize: '13px' }}>{sanitize(cmd.desc)}</td>
                  ...
                  <td style={{ padding: '12px 16px', fontSize: '11px', color: '#64748B' }}>{sanitize(cmd.allowedRoles) || 'Все'}</td>
