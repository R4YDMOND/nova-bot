...
const INITIAL_COMMANDS: Command[] = [
  ...
  { name: '🧹 /clear', desc: 'Очистить чат',          category: 'Модерация', enabled: true,  cooldown: 5,  allowedRoles: '@админ' },
  ...
  { name: '🎵 /play',  desc: 'Музыка',                category: 'Музыка',    enabled: true,  cooldown: 3,  allowedRoles: '@DJ' },
  ...
  return (
    <div className="space-y-6 p-8 max-w-5xl">
      <div className="flex justify-between items-start flex-wrap gap-4">
        <div>
          <h1 className="text-3xl font-bold text-[rgb(var(--text))]">⚡ Команды</h1>
            ...
            <button key={cat} onClick={() => setActiveCategory(cat)}
              className={`px-4 py-2 rounded-2xl text-sm font-medium border transition-all ${
                activeCategory === cat
                  ...
                  : 'border-transparent text-[rgb(var(--text-secondary))] hover:text-[rgb(var(--text))] hover:bg-[rgb(var(--surface-2))]'
              }`}>
              {cat}
            </button>
          ))}
              ...
              {filtered.length === 0 ? (
                ...
                <tr key={i} className="border-b border-[rgb(var(--border))] hover:bg-[rgb(var(--surface-2))] transition-colors">
                  <td className="py-4 px-4">
                    <code className="bg-[rgb(var(--surface-2))] px-2.5 py-1 rounded-lg text-primary font-mono text-xs">
                      {cmd.name}
                    </code>
                  </td>
                  <td className="py-4 px-4 text-[rgb(var(--text-secondary))]">{cmd.desc}</td>
                  <td className="py-4 px-4">
                  ...
                  <td className="py-4 px-4 text-xs text-[rgb(var(--text-secondary))]">
