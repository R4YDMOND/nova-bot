...
export default function WebhooksPage() {
  ...
  const toggle = function(id: string) {
    setWebhooks(webhooks.map(function(w) { return w.id === id ? { ...w, active: !w.active } : w }))
  }

  const deleteWebhook = function(id: string) {
    setWebhooks(webhooks.filter(function(w) { return w.id !== id }))
  }

  const toggleRule = function(webhookId: string, rule: string) {
    setWebhooks(webhooks.map(function(w) {
      if (w.id !== webhookId) return w
      var newRules = { ...w.rules, [rule]: !w.rules[rule as keyof typeof w.rules] }
      return { ...w, rules: newRules }
    }))
  }

  const updateWebhook = function(id: string, key: string, value: any) {
    setWebhooks(webhooks.map(function(w) { return w.id === id ? { ...w, [key]: value } : w }))
  }

  const detectPlatform = function(url: string): string {
    if (!url) return ''
    var clean = url.toLowerCase().trim()
    if (clean.includes('lolka.app') || clean.includes('lolka')) return 'Lolka'
    if (clean.includes('vk.com') || clean.includes('vk.ru')) return 'VK'
    return ''
  }

  const handleUrlChange = function(value: string) {
    var platform = detectPlatform(value)
    setNewWebhook({ ...newWebhook, url: value, platform: platform || newWebhook.platform })
  }

  const channelsFor = function(platform: string): string[] {
    return mockChannels[platform] || []
  }
  ...
  const scanWebhook = async function(wh: Webhook) {
    setScanning(true)
    setScanResult('')
    var data = await apiCall(API_BASE + '/api/scan/content', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ platform: wh.platform.toLowerCase(), group_id: wh.groupId, serverName: wh.serverName, min_likes: wh.minLikes, rules: wh.rules, webhooks: [{ platform: wh.platform.toLowerCase(), url: wh.url, group_id: wh.groupId, rules: wh.rules, serverName: wh.serverName, channel: wh.channel }] })
    })
    setScanResult(data.found ? '✅ Найдено: ' + data.found + ', переслано: ' + (data.sent || 0) : 'ℹ️ Нет нового')
    setScanning(false)
    setTimeout(function() { setScanResult('') }, 4000)
  }

  const scanAll = function() {
    webhooks.filter(function(w) { return w.active }).forEach(function(wh, i) {
      setTimeout(function() { scanWebhook(wh) }, i * 500)
    })
  }
  ...
  const sendBroadcast = async function() {
    ...
    var data = await apiCall(API_BASE + '/api/send/broadcast', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ message: broadcastMsg, platforms: broadcastPlatforms, webhooks: active.map(function(w) { return { platform: w.platform.toLowerCase(), url: w.url, group_id: w.groupId, serverName: w.serverName } }) })
    })
    setSendResult('✅ ' + data.sent_count + '/' + data.total)
    setSending(false)
  ...
  const forwardContent = async function() {
    ...
    var data = await apiCall(API_BASE + '/api/forward', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ message: ('/forward ' + forwardUrl + ' ' + forwardComment).trim(), platform: active[0]?.platform || 'Lolka', serverName: active[0]?.serverName || '', webhooks: active.map(function(w) { return { platform: w.platform.toLowerCase(), url: w.url, group_id: w.groupId, rules: w.rules, serverName: w.serverName, channel: w.channel } }) })
    })
    setForwardResult('✅ ' + data.total_sent + '/' + data.total_urls)
    setSending(false)
    setTimeout(function() { setForwardResult('') }, 4000)
  }
    ...
    <div style={{ minHeight: '100vh', background: '#0A0A0F', display: 'flex', color: '#F1F5F9' }}>
      ...
      <aside style={{ width: '240px', minWidth: '240px', background: '#111118', borderRight: '1px solid #1F2937', padding: '24px 16px', display: 'flex', flexDirection: 'column' }}>
        ...
        <nav style={{ display: 'flex', flexDirection: 'column', gap: '2px', flex: 1 }}>
          {[
            ...
            return (
              <span key={i} onClick={function() { navigate(item.href) }} style={{ padding: '10px 12px', borderRadius: '8px', display: 'flex', alignItems: 'center', gap: '10px', fontSize: '14px', cursor: 'pointer', fontWeight: isActive ? '500' : '400', color: isActive ? '#FFF' : '#94A3B8', background: isActive ? '#1F2937' : 'transparent', transition: 'all 0.15s' }}>
      ...
      <main style={{ flex: 1, padding: '32px 40px', overflow: 'auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '10px' }}>
          <div><h1 style={{ fontSize: '26px', fontWeight: '700', marginBottom: '2px' }}>🔗 Вебхуки</h1><p style={{ color: '#94A3B8', fontSize: '13px' }}>Автоопределение, каналы, автосканирование, пересылка</p></div>
        ...
        {showForward && (
          <div style={{ background: '#16161F', borderRadius: '14px', padding: '20px', marginBottom: '16px', border: '1px solid #EC489920' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', marginBottom: '10px', flexWrap: 'wrap' }}>
              <input type="text" value={forwardUrl} onChange={function(e) { setForwardUrl(e.target.value) }} placeholder="https://ссылка..." style={{ flex: 1, minWidth: '250px', padding: '10px 14px', background: '#0A0A0F', border: '1px solid #1F2937', borderRadius: '10px', color: '#FFF', fontSize: '13px', outline: 'none' }} />
              <select value={forwardType} onChange={function(e) { setForwardType(e.target.value) }} style={{ padding: '10px 14px', background: '#0A0A0F', border: '1px solid #1F2937', borderRadius: '10px', color: '#FFF', fontSize: '13px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
              <input type="text" value={forwardComment} onChange={function(e) { setForwardComment(e.target.value) }} placeholder="💬 Комментарий" style={{ flex: 1, padding: '10px 14px', background: '#0A0A0F', border: '1px solid #1F2937', borderRadius: '10px', color: '#FFF', fontSize: '13px', outline: 'none' }} />
        ...
        {showBroadcast && (
          <div style={{ background: '#16161F', borderRadius: '14px', padding: '20px', marginBottom: '16px', border: '1px solid #A855F720' }}>
            ...
            <div style={{ display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
              {['lolka', 'vk'].map(function(p) {
                return (
                  <label key={p} style={{ display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer', fontSize: '13px' }}>
                    <input type="checkbox" checked={broadcastPlatforms.includes(p)} onChange={function() { setBroadcastPlatforms(function(prev) { return prev.includes(p) ? prev.filter(function(x) { return x !== p }) : [...prev, p] }) }} />
                    {p === 'lolka' ? '🎮' : '💙'} {p}
                  </label>
                )
              })}
              ...
              <div key={w.id} style={{ background: '#16161F', borderRadius: '14px', padding: '16px 20px', border: '1px solid #1F2937', opacity: w.active ? 1 : 0.5 }}>
                ...
                {editingScan === w.id && (
                  <div style={{ background: '#111118', borderRadius: '10px', padding: '14px', display: 'flex', gap: '14px', flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    <label style={{ display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer', fontSize: '12px' }}><input type="checkbox" checked={w.autoScan} onChange={function() { updateWebhook(w.id, 'autoScan', !w.autoScan) }} style={{ accentColor: '#10B981' }} /> Вкл</label>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><span style={{ fontSize: '12px', color: '#94A3B8' }}>Интервал:</span><select value={w.scanInterval} onChange={function(e) { updateWebhook(w.id, 'scanInterval', parseInt(e.target.value)) }} style={{ padding: '4px 8px', background: '#0A0A0F', border: '1px solid #1F2937', borderRadius: '6px', color: '#FFF', fontSize: '12px' }}><option value="15">15м</option><option value="30">30м</option><option value="60">1ч</option><option value="120">2ч</option></select></div>
        ...
        <div style={{ background: '#16161F', borderRadius: '14px', padding: '20px', marginTop: '20px', border: '1px solid #1F2937' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
            <div style={{ background: '#111118', borderRadius: '10px', padding: '14px' }}><div style={{ fontWeight: '600', fontSize: '13px', marginBottom: '4px' }}>🔍 Автоопределение</div><p style={{ fontSize: '12px', color: '#94A3B8', lineHeight: '1.5' }}>Вставьте URL — платформа определится автоматически.</p></div>
            <div style={{ background: '#111118', borderRadius: '10px', padding: '14px' }}><div style={{ fontWeight: '600', fontSize: '13px', marginBottom: '4px' }}>📋 Выбор канала</div><p style={{ fontSize: '12px', color: '#94A3B8', lineHeight: '1.5' }}>Выберите канал из списка или введите свой.</p></div>
          </div>
        </div>
