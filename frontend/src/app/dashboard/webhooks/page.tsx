...
export default function WebhooksPage() {
  ...
  // Автоопределение канала из ссылки
  const detectChannelFromLink = (link: string) => {
    if (!link) return ''
    // Lolka: https://lolka.app/channels/xxx или #канал
    if (link.includes('lolka.app/channels/')) {
      return link.split('/').pop() || ''
    }
    // VK: https://vk.com/club123 или #чат
  ...
  const handleChannelLink = (link: string) => {
    ...
    if (detected) {
      setNewEvent({ ...newEvent, channel: '#' + detected })
    }
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0A0A0F', display: 'flex', color: '#F1F5F9' }}>
      <aside style={{ width: '220px', minWidth: '220px', background: '#111118', borderRight: '1px solid #1F2937', padding: '20px 14px', display: 'flex', flexDirection: 'column' }}>
        <a href="/" style={{ display: 'flex', alignItems: 'center', gap: '8px', textDecoration: 'none', marginBottom: '28px' }}>
          <div style={{ width: '30px', height: '30px', background: '#16161F', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', color: '#00E5FF', fontSize: '15px' }}>N</div>
          <span style={{ fontSize: '17px', fontWeight: 'bold', color: '#FFF' }}>Нова</span>
        </a>
        <nav style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
          {[
            ...
            var isActive = item.label === 'Вебхуки';
            return (
              <span key={i} onClick={function() { navigate(item.href) }}
                style={{ padding: '8px 10px', borderRadius: '6px', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', cursor: 'pointer', fontWeight: isActive ? '500' : '400', color: isActive ? '#FFF' : '#94A3B8', background: isActive ? '#1F2937' : 'transparent' }}>
      ...
      <main style={{ flex: 1, padding: '28px 36px', overflow: 'auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px', flexWrap: 'wrap', gap: '10px' }}>
          ...
          <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
            <button onClick={function() { setShowEvents(!showEvents); if (!showEvents) loadEvents() }}
              style={btnStyle(showEvents, '#EF4444')}>{showEvents ? '✕' : '🗓️ События'}</button>
            <button onClick={function() { setShowLogs(!showLogs); if (!showLogs) loadLogs() }}
              style={btnStyle(showLogs, '#F59E0B')}>{showLogs ? '✕' : '📋 Логи'}</button>
            <button onClick={function() { setShowAutoForward(!showAutoForward) }}
              style={btnStyle(showAutoForward, '#10B981')}>{showAutoForward ? '✕' : '🔄 Форвард'}</button>
            <button onClick={function() { setShowAdd(!showAdd) }}
              style={btnStyle(showAdd, '#00E5FF')}>{showAdd ? '✕' : '+ Вебхук'}</button>
          </div>
        </div>
        ...
        {showEvents && (
          <div style={panelStyle('#EF444410')}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
              ...
              <button onClick={function() { setShowAddEvent(!showAddEvent) }}
                style={{ padding: '6px 12px', background: showAddEvent ? '#1F2937' : '#EF4444', color: '#FFF', border: 'none', borderRadius: '6px', fontWeight: '600', cursor: 'pointer', fontSize: '11px' }}>
                {showAddEvent ? '✕' : '+ Создать'}
              </button>
            </div>
            ...
            {showAddEvent && (
              <div style={{ background: '#111118', borderRadius: '10px', padding: '16px', marginBottom: '12px' }}>
                ...
                <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: '12px' }}>
                  {templates.map((t: any) => (
                    <button key={t.value} onClick={function() { selectTemplate(t.value) }}
                      style={{
                        padding: '5px 10px', borderRadius: '6px', border: '1px solid ' + (newEvent.template === t.value ? getEventColor[t.value] : '#1F2937'),
                        background: newEvent.template === t.value ? getEventColor[t.value] + '20' : 'transparent',
                        color: newEvent.template === t.value ? getEventColor[t.value] : '#94A3B8',
                        cursor: 'pointer', fontSize: '11px', fontWeight: newEvent.template === t.value ? '600' : '400'
                      }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '8px' }}>
                  <input type="text" value={newEvent.title} onChange={function(e) { setNewEvent({ ...newEvent, title: e.target.value }) }}
                    placeholder="Название события" style={inputStyle} />
                  <input type="datetime-local" value={newEvent.event_date} onChange={function(e) { setNewEvent({ ...newEvent, event_date: e.target.value }) }}
                    style={inputStyle} />
                </div>

                {/* Канал — автоопределение */}
                <div style={{ marginBottom: '8px' }}>
                  <label style={{ fontSize: '10px', color: '#64748B', display: 'block', marginBottom: '4px' }}>
                    💡 Вставьте ссылку на канал — название подтянется автоматически
                  </label>
                  <div style={{ display: 'flex', gap: '6px' }}>
                    <input
                      type="text"
                      value={channelLinkInput}
                      onChange={function(e) { handleChannelLink(e.target.value) }}
                      placeholder="https://lolka.app/channels/... или #канал"
                      style={{ ...inputStyle, flex: 1 }}
                    />
                    <input
                      type="text"
                      value={newEvent.channel}
                      onChange={function(e) { setNewEvent({ ...newEvent, channel: e.target.value }) }}
                      placeholder="Канал"
                    ...
                    <input
                      ...
                      onChange={function(e) { setNewEvent({ ...newEvent, max_participants: parseInt(e.target.value) || 0 }) }}
                      placeholder="Мест"
                ...
                <textarea value={newEvent.description} onChange={function(e) { setNewEvent({ ...newEvent, description: e.target.value }) }}
                  placeholder="Описание события..." rows={2}
                  style={{ ...inputStyle, width: '100%', resize: 'vertical', marginBottom: '10px', boxSizing: 'border-box' }} />

                <button onClick={createEvent}
                  style={{ width: '100%', padding: '10px', background: '#EF4444', color: '#FFF', border: 'none', borderRadius: '8px', fontWeight: '600', cursor: 'pointer', fontSize: '13px' }}>
                  ✅ Создать событие
                </button>
              </div>
            )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              {events.length === 0 && (
                <p style={{ color: '#64748B', fontSize: '12px', textAlign: 'center', padding: '16px' }}>Нет запланированных событий</p>
              )}
                  ...
                  <div key={e.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 14px', background: '#111118', borderRadius: '8px', borderLeft: '3px solid ' + color }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: '10px', color: '#94A3B8' }}>
                        ...
                        {e.channel ? ' • ' + e.channel : ''}
                        {e.max_participants > 0 ? ' • ' + e.max_participants + ' мест' : ''}
                      </div>
                    </div>
                    <button onClick={function() { notifyEvent(e) }}
                      style={{ padding: '4px 8px', background: color + '20', color: color, border: '1px solid ' + color + '40', borderRadius: '5px', cursor: 'pointer', fontSize: '11px', whiteSpace: 'nowrap' }}>
                      📢
                    </button>
                    <button onClick={function() { deleteEvent(e.id) }}
                      style={{ padding: '4px 6px', background: 'transparent', color: '#EF4444', border: '1px solid #374151', borderRadius: '5px', cursor: 'pointer', fontSize: '11px' }}>
                      ✕
                    </button>
                  </div>
                );
        ...
        {showLogs && (
          <div style={panelStyle('#F59E0B10')}>
            ...
            {logsLoading ? <p style={{ color: '#94A3B8', fontSize: '12px' }}>⏳ Загрузка...</p> :
              logs.length === 0 ? <p style={{ color: '#64748B', fontSize: '12px' }}>Логов пока нет</p> :
          ...
          <div style={panelStyle('#10B98110')}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div onClick={function() { setAutoForward({ ...autoForward, enabled: !autoForward.enabled }) }}
                  style={{ width: '40px', height: '22px', background: autoForward.enabled ? '#10B981' : '#374151', borderRadius: '22px', cursor: 'pointer', position: 'relative' }}>
                  <div style={{ position: 'absolute', height: '16px', width: '16px', left: autoForward.enabled ? '22px' : '3px', top: '3px', background: '#FFF', borderRadius: '50%', transition: '0.2s' }} />
                </div>
              </div>
              ...
              <button onClick={saveAutoForward}
                style={{ padding: '8px 16px', background: '#10B981', color: '#000', border: 'none', borderRadius: '8px', fontWeight: '600', cursor: 'pointer', fontSize: '12px', marginTop: '4px', width: 'fit-content' }}>
                💾 Сохранить
              </button>
            </div>
          </div>
        ...
        <div style={{ marginTop: showEvents || showLogs || showAutoForward ? '16px' : '0' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
            ...
            <div style={{ display: 'flex', gap: '4px' }}>
              <button onClick={scanAll} disabled={scanning}
                style={{ padding: '5px 10px', background: scanning ? '#374151' : '#3B82F620', color: scanning ? '#64748B' : '#3B82F6', border: '1px solid ' + (scanning ? '#374151' : '#3B82F640'), borderRadius: '6px', cursor: scanning ? 'wait' : 'pointer', fontSize: '11px' }}>
                {scanning ? '⏳' : '🔍 Сканировать всё'}
              </button>
              <button onClick={function() { setShowForward(!showForward) }}
                style={{ padding: '5px 10px', background: '#EC489920', color: '#EC4899', border: '1px solid #EC489940', borderRadius: '6px', cursor: 'pointer', fontSize: '11px' }}>
                ↗
              </button>
              <button onClick={function() { setShowBroadcast(!showBroadcast) }}
                style={{ padding: '5px 10px', background: '#A855F720', color: '#A855F7', border: '1px solid #A855F740', borderRadius: '6px', cursor: 'pointer', fontSize: '11px' }}>
                📢
              </button>
            </div>
          </div>

          {/* Forward + Broadcast панели */}
          {showForward && (
            <div style={{ ...panelStyle('#EC489910'), marginBottom: '10px', padding: '14px' }}>
              <div style={{ display: 'flex', gap: '6px', marginBottom: '6px', flexWrap: 'wrap' }}>
                <input type="text" value={forwardUrl} onChange={function(e) { setForwardUrl(e.target.value) }} placeholder="https://ссылка..." style={{ ...inputStyle, flex: 1, minWidth: '200px' }} />
                <select value={forwardType} onChange={function(e) { setForwardType(e.target.value) }} style={{ ...inputStyle, width: '110px' }}>
              ...
              <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                <input type="text" value={forwardComment} onChange={function(e) { setForwardComment(e.target.value) }} placeholder="💬 Комментарий" style={{ ...inputStyle, flex: 1 }} />
                <button onClick={forwardContent} disabled={sending || !forwardUrl.trim()}
                  style={{ padding: '6px 14px', background: sending ? '#374151' : '#EC4899', color: '#FFF', border: 'none', borderRadius: '6px', fontWeight: '600', cursor: sending ? 'wait' : 'pointer', fontSize: '12px', whiteSpace: 'nowrap' }}>
                  {sending ? '⏳' : '🚀 Переслать'}
                </button>
              </div>
          ...
          {showBroadcast && (
            <div style={{ ...panelStyle('#A855F710'), marginBottom: '10px', padding: '14px' }}>
              <textarea value={broadcastMsg} onChange={function(e) { setBroadcastMsg(e.target.value) }} placeholder="Текст рассылки..." rows={2}
                style={{ width: '100%', padding: '8px 12px', background: '#0A0A0F', border: '1px solid #1F2937', borderRadius: '8px', color: '#FFF', fontSize: '13px', resize: 'vertical', boxSizing: 'border-box', marginBottom: '8px' }} />
              <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
                {['lolka', 'vk'].map(function(p) { return (
                  <label key={p} style={{ display: 'flex', alignItems: 'center', gap: '4px', cursor: 'pointer', fontSize: '12px' }}>
                    <input type="checkbox" checked={broadcastPlatforms.includes(p)} onChange={function() { setBroadcastPlatforms(function(prev) { return prev.includes(p) ? prev.filter(function(x) { return x !== p }) : [...prev, p] }) }} style={{ accentColor: '#A855F7' }} />
                    {p === 'lolka' ? '🎮 Lolka' : '💙 VK'}
                  </label>
                )})}
                <button onClick={sendBroadcast} disabled={sending || !broadcastMsg.trim()}
                  style={{ padding: '6px 16px', background: sending ? '#374151' : '#A855F7', color: '#FFF', border: 'none', borderRadius: '6px', fontWeight: '600', cursor: sending ? 'wait' : 'pointer', fontSize: '12px' }}>
                  {sending ? '⏳' : '🚀 Отправить'}
                </button>
            ...
            {webhooks.map(function(w) { return (
              <div key={w.id} style={{ background: '#16161F', borderRadius: '10px', padding: '12px 16px', border: '1px solid #1F2937', opacity: w.active ? 1 : 0.4 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                  {/* Платформа */}
                  <span style={{ padding: '3px 8px', borderRadius: '5px', fontSize: '11px', fontWeight: '600', background: w.platform === 'Lolka' ? 'rgba(88,101,242,0.12)' : 'rgba(0,119,255,0.12)', color: w.platform === 'Lolka' ? '#5865F2' : '#0077FF' }}>
                  ...
                  {/* Канал */}
                  <div onClick={function() { setEditingChannel(editingChannel === w.id ? null : w.id) }}
                    style={{ fontSize: '11px', color: '#94A3B8', cursor: 'pointer', padding: '3px 8px', borderRadius: '5px', background: '#0A0A0F', border: '1px solid #1F2937', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: '4px' }}>
                    <span>{w.channel || '📋 выбрать'}</span><span style={{ fontSize: '8px' }}>▾</span>
                  </div>
                  {/* Правила */}
                  <div style={{ display: 'flex', gap: '3px' }}>
                    {Object.entries(w.rules).filter(function(e) { return e[1] }).map(function(e) {
                      return <span key={e[0]} style={{ fontSize: '12px' }} title={getTypeLabel[e[0]] || e[0]}>{getTypeIcon[e[0]] || '📎'}</span>
                    })}
                  </div>
                  {/* Переключатель */}
                  <div onClick={function() { toggle(w.id) }}
                    style={{ width: '32px', height: '18px', background: w.active ? '#00E5FF' : '#374151', borderRadius: '18px', cursor: 'pointer', position: 'relative', flexShrink: 0 }}>
                    <div style={{ position: 'absolute', height: '12px', width: '12px', left: w.active ? '18px' : '2px', top: '3px', background: w.active ? '#000' : '#FFF', borderRadius: '50%', transition: '0.2s' }} />
                  </div>
                  {/* Кнопки действий */}
                  <button onClick={function() { setEditingRules(editingRules === w.id ? null : w.id) }}
                    style={actionBtnStyle('#F59E0B')}>⚙️</button>
                  <button onClick={function() { setEditingScan(editingScan === w.id ? null : w.id) }}
                    style={actionBtnStyle('#10B981')}>🔍</button>
                  <button onClick={function() { testWebhook(w) }}
                    style={actionBtnStyle('#00E5FF')}>🧪</button>
                  <button onClick={function() { deleteWebhook(w.id) }}
                    style={actionBtnStyle('#EF4444')}>✕</button>
                </div>
                {/* Выбор канала */}
                {editingChannel === w.id && (
                  <div style={{ background: '#111118', borderRadius: '8px', padding: '10px', marginTop: '8px', display: 'flex', gap: '6px', flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    {channelsFor(w.platform).map(function(ch) {
                      return <button key={ch} onClick={function() { updateWebhook(w.id, 'channel', ch); setEditingChannel(null) }}
                        style={{ padding: '4px 10px', borderRadius: '6px', border: '1px solid #1F2937', background: w.channel === ch ? 'rgba(0,229,255,0.1)' : 'transparent', color: w.channel === ch ? '#00E5FF' : '#94A3B8', cursor: 'pointer', fontSize: '11px', whiteSpace: 'nowrap' }}>{ch}</button>
                    })}
                    <input type="text" placeholder="Свой..." onKeyDown={function(e) { if (e.key === 'Enter' && (e.target as HTMLInputElement).value.trim()) { updateWebhook(w.id, 'channel', (e.target as HTMLInputElement).value.trim()); setEditingChannel(null) } }}
                      style={{ padding: '4px 8px', background: '#0A0A0F', border: '1px solid #1F2937', borderRadius: '6px', color: '#FFF', fontSize: '11px', width: '100px' }} />
                  </div>
                )}
                {/* Правила */}
                {editingRules === w.id && (
                  <div style={{ background: '#111118', borderRadius: '8px', padding: '10px', marginTop: '8px', display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    {Object.entries(w.rules).map(function(e) { var key = e[0]; var value = e[1]; return (
                      <label key={key} style={{ display: 'flex', alignItems: 'center', gap: '4px', cursor: 'pointer', fontSize: '11px', padding: '4px 10px', borderRadius: '6px', background: value ? 'rgba(0,229,255,0.1)' : '#0A0A0F', border: '1px solid ' + (value ? '#00E5FF40' : '#1F2937') }}>
                        <input type="checkbox" checked={value} onChange={function() { toggleRule(w.id, key) }} style={{ accentColor: '#00E5FF' }} />
                        {getTypeIcon[key] || '📎'} {getTypeLabel[key] || key}
                      </label>
                    )})}
                  </div>
                )}
                ...
                {editingScan === w.id && (
                  <div style={{ background: '#111118', borderRadius: '8px', padding: '10px', marginTop: '8px', display: 'flex', gap: '12px', flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    <label style={{ display: 'flex', alignItems: 'center', gap: '4px', cursor: 'pointer', fontSize: '11px' }}>
                      <input type="checkbox" checked={w.autoScan} onChange={function() { updateWebhook(w.id, 'autoScan', !w.autoScan) }} style={{ accentColor: '#10B981' }} /> Вкл
                    </label>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                      <span style={{ fontSize: '11px', color: '#94A3B8' }}>Интервал:</span>
                      <select value={w.scanInterval} onChange={function(e) { updateWebhook(w.id, 'scanInterval', parseInt(e.target.value)) }}
                        style={{ padding: '3px 6px', background: '#0A0A0F', border: '1px solid #1F2937', borderRadius: '5px', color: '#FFF', fontSize: '11px' }}>
                        <option value="15">15м</option><option value="30">30м</option><option value="60">1ч</option><option value="120">2ч</option>
                      </select>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                      <span style={{ fontSize: '11px', color: '#94A3B8' }}>Мин 👍:</span>
                      <input type="number" value={w.minLikes} onChange={function(e) { updateWebhook(w.id, 'minLikes', parseInt(e.target.value) || 1) }}
                        style={{ width: '45px', padding: '3px 6px', background: '#0A0A0F', border: '1px solid #1F2937', borderRadius: '5px', color: '#FFF', fontSize: '11px' }} />
                    </div>
                  </div>
                )}
              </div>
        ...
        {saved && (
          <div style={{ position: 'fixed', bottom: '20px', right: '20px', background: '#22C55E', color: '#000', padding: '10px 20px', borderRadius: '10px', fontWeight: '600', fontSize: '13px', zIndex: 1000, boxShadow: '0 4px 20px rgba(34,197,94,0.3)', animation: 'slideUp 0.3s ease' }}>
            ✅ Сохранено
          </div>
        )}
...
const btnStyle = (active: boolean, color: string): React.CSSProperties => ({
  padding: '7px 14px',
  background: active ? '#1F2937' : color + '15',
  color: active ? '#FFF' : color,
  border: '1px solid ' + (active ? '#374151' : color + '30'),
  borderRadius: '8px',
  fontWeight: '600',
  cursor: 'pointer',
  fontSize: '12px',
