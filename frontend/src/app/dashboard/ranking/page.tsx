  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0A0A0F', color: '#F1F5F9' }}>
      ...
      <div style={{ display: 'flex', gap: '4px', padding: '16px 40px', background: '#0A0A0F', borderBottom: '1px solid #1F2937', overflowX: 'auto' }}>
        {TABS.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{ padding: '10px 20px', borderRadius: '10px', border: 'none', background: activeTab === tab.id ? '#1F2937' : 'transparent', color: activeTab === tab.id ? '#FFF' : '#94A3B8', fontWeight: activeTab === tab.id ? '600' : '400', cursor: 'pointer', fontSize: '14px', transition: 'all 0.2s', whiteSpace: 'nowrap' }}>{sanitize(tab.label)}</button>
        ))}
      </div>
      ...
      <main style={{ padding: '40px', maxWidth: '1100px', margin: '0 auto' }}>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
              <div style={{ background: '#16161F', borderRadius: '18px', padding: '28px', border: '1px solid #1F2937' }}>
                ...
                <div style={{ marginBottom: '16px' }}><label style={{ fontSize: '12px', color: '#94A3B8', display: 'block', marginBottom: '6px' }}>Цвет фона</label><div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}><input type="color" value={cardBgColor} onChange={(e) => setCardBgColor(e.target.value)} style={{ width: '36px', height: '36px', border: 'none', borderRadius: '8px', cursor: 'pointer' }} /><input type="text" value={sanitize(cardBgColor)} onChange={(e) => setCardBgColor(e.target.value)} style={{ flex: 1, padding: '8px', background: '#0A0A0F', border: '1px solid #1F2937', borderRadius: '8px', color: '#FFF', fontSize: '14px' }} /></div></div>
                <div style={{ marginBottom: '16px' }}><label style={{ fontSize: '12px', color: '#94A3B8', display: 'block', marginBottom: '6px' }}>Акцентный цвет</label><div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}><input type="color" value={cardAccentColor} onChange={(e) => setCardAccentColor(e.target.value)} style={{ width: '36px', height: '36px', border: 'none', borderRadius: '8px', cursor: 'pointer' }} /><input type="text" value={sanitize(cardAccentColor)} onChange={(e) => setCardAccentColor(e.target.value)} style={{ flex: 1, padding: '8px', background: '#0A0A0F', border: '1px solid #1F2937', borderRadius: '8px', color: '#FFF', fontSize: '14px' }} /></div></div>
                <div style={{ marginBottom: '16px' }}><label style={{ fontSize: '12px', color: '#94A3B8', display: 'block', marginBottom: '6px' }}>Никнейм (макс. 25 символов)</label><input type="text" value={sanitize(cardNickname)} onChange={(e) => setCardNickname(e.target.value.slice(0, 25))} placeholder="Пользователь" maxLength={25} style={{ width: '100%', padding: '10px', background: '#0A0A0F', border: '1px solid #1F2937', borderRadius: '8px', color: '#FFF', fontSize: '14px', boxSizing: 'border-box' }} /></div>
                <div style={{ marginBottom: '16px' }}><label style={{ fontSize: '12px', color: '#94A3B8', display: 'block', marginBottom: '6px' }}>URL аватара (VK/Lolka)</label><input type="text" value={sanitize(cardAvatarUrl)} onChange={(e) => setCardAvatarUrl(e.target.value)} placeholder="https://..." style={{ width: '100%', padding: '10px', background: '#0A0A0F', border: '1px solid #1F2937', borderRadius: '8px', color: '#FFF', fontSize: '14px', boxSizing: 'border-box' }} /></div>
                <div><label style={{ fontSize: '12px', color: '#94A3B8', display: 'block', marginBottom: '6px' }}>Стиль</label><select value={cardStyle} onChange={(e) => setCardStyle(e.target.value)} style={{ width: '100%', padding: '10px', background: '#0A0A0F', border: '1px solid #1F2937', borderRadius: '8px', color: '#FFF', fontSize: '14px' }}>{cardStyles.map(s => <option key={s.value} value={s.value}>{s.label}</option>)}</select></div>
              </div>
              <div style={{ background: '#16161F', borderRadius: '18px', padding: '28px', border: '1px solid #1F2937' }}>
                ...
                {[
                  ...
                  { key: 'cardShowAchievements', label: '🏅 Достижения', value: cardShowAchievements, set: setCardShowAchievements },
                  { key: 'cardShowShare', label: '🔗 Поделиться', value: cardShowShare, set: setCardShowShare },
                ].map((item, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: i < 4 ? '1px solid #1F2937' : 'none' }}><span style={{ fontSize: '14px' }}>{sanitize(item.label)}</span><div onClick={() => item.set(!item.value)} style={{ width: '44px', height: '26px', background: item.value ? '#00E5FF' : '#374151', borderRadius: '26px', cursor: 'pointer', position: 'relative' }}><div style={{ position: 'absolute', height: '20px', width: '20px', left: item.value ? '22px' : '4px', top: '3px', background: item.value ? '#000' : '#FFF', borderRadius: '50%', transition: '0.25s' }} /></div></div>
                ))}
              </div>
            </div>
            ...
            <div style={{ marginTop: '24px' }}><h3 style={{ fontSize: '16px', fontWeight: '600', marginBottom: '16px' }}>👁️ Предпросмотр карточки</h3>
              <div style={{ ...getCardStyle(), borderRadius: '16px', padding: '24px', position: 'relative', overflow: 'hidden', maxWidth: '400px' }}>
                ...
                <div style={{ position: 'relative', zIndex: 1 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>{cardShowRank && <div style={{ background: cardAccentColor, color: '#000', fontWeight: 'bold', padding: '4px 12px', borderRadius: '8px', fontSize: '12px' }}>🏆 #1 в рейтинге</div>}<div style={{ fontSize: '11px', color: '#94A3B8' }}>Ур. 42</div></div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                    {cardShowAvatar && (<div style={{ width: '56px', height: '56px', minWidth: '56px', borderRadius: '16px', border: `3px solid ${cardAccentColor}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '28px', position: 'relative', overflow: 'hidden' }}>{cardAvatarUrl ? <img src={cardAvatarUrl} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} onError={(e) => { e.currentTarget.style.display = 'none' }} /> : <span>👤</span>}<div style={{ position: 'absolute', bottom: '-2px', right: '-2px', width: '14px', height: '14px', background: '#22C55E', borderRadius: '50%', border: '2px solid #000' }} /></div>)}
                    <div style={{ flex: 1 }}>
                      ...
                      {cardShowXP && (<div><div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '10px', color: '#94A3B8', marginBottom: '4px' }}><span>XP</span><span>15,420 / 20,000</span></div><div style={{ height: '8px', background: 'rgba(255,255,255,0.1)', borderRadius: '4px', overflow: 'hidden' }}><div style={{ width: '77%', height: '100%', background: `linear-gradient(90deg, ${cardAccentColor}, ${cardAccentColor}88)`, borderRadius: '4px' }} /></div></div>)}
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: '8px', marginTop: '14px' }}><div style={{ flex: 1, background: 'rgba(255,255,255,0.05)', borderRadius: '8px', padding: '6px 10px', textAlign: 'center' }}><div style={{ fontSize: '10px', color: '#94A3B8' }}>💬 Сообщений</div><div style={{ fontSize: '14px', fontWeight: 'bold', color: cardAccentColor }}>2.4K</div></div><div style={{ flex: 1, background: 'rgba(255,255,255,0.05)', borderRadius: '8px', padding: '6px 10px', textAlign: 'center' }}><div style={{ fontSize: '10px', color: '#94A3B8' }}>🎤 Голос</div><div style={{ fontSize: '14px', fontWeight: 'bold', color: cardAccentColor }}>120ч</div></div><div style={{ flex: 1, background: 'rgba(255,255,255,0.05)', borderRadius: '8px', padding: '6px 10px', textAlign: 'center' }}><div style={{ fontSize: '10px', color: '#94A3B8' }}>⭐ Реакций</div><div style={{ fontSize: '14px', fontWeight: 'bold', color: cardAccentColor }}>856</div></div></div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '32px' }}>
          ...
          <div style={{ display: 'flex', gap: '12px' }}><button onClick={resetAll} style={{ padding: '12px 20px', background: 'transparent', color: '#EF4444', border: '1px solid #374151', borderRadius: '12px', fontWeight: '600', cursor: 'pointer', fontSize: '14px' }}>🔄 Сбросить</button><button onClick={save} style={{ padding: '12px 28px', background: saved ? '#22C55E' : '#00E5FF', color: '#000', border: 'none', borderRadius: '12px', fontWeight: '600', cursor: 'pointer', fontSize: '14px', transition: 'all 0.3s', boxShadow: saved ? '0 0 20px rgba(34,197,94,0.3)' : '0 0 20px rgba(0,229,255,0.2)' }}>{saved ? '✅ Сохранено!' : '💾 Сохранить всё'}</button></div>
        </div>
