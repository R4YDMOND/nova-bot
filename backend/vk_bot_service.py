...
class VKBotService:
    ...
    def delete_by_cmid(self, peer_id: int, cmid: int, delete_for_all: bool = True) -> int:
        """
        Удалить сообщение по conversation_message_id — используется, когда обычный
        message_id недоступен (например, в message_event — см. VK - Документация.md,
        раздел "Работа с conversation_message_id": messages.delete с cmids+peer_id).
        """
        params = {
            "peer_id": peer_id,
            "cmids": cmid,
    ...
    def answer_message_event(
        ...
        """
        Ответ на клик по callback-кнопке (событие message_event) — обязателен,
        иначе кнопка «зависает» с крутящимся индикатором на стороне клиента.
        event_data опционален: без него VK просто снимает индикатор загрузки.
        """
        params: Dict[str, Any] = {
            "event_id": event_id,
            "user_id": user_id,
        ...
        resp = self._call("messages.sendMessageEventAnswer", params)
        return resp
