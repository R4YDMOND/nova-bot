...
class XPFormulaEngine:
    # ИСПРАВЛЕНО: Callable с большой буквы
    ...
    def calculate_xp(config: XPFormulaConfig, current_level: int, message_length: int, is_voice: bool = False) -> int:
        ...
        if message_length > 50: xp *= 1.2
        elif message_length > 100: xp *= 1.5
        if is_voice: xp *= 1.5
        if config.decay_factor > 0: xp *= (1 - config.decay_factor)
    ...
    def calculate_level_xp(level: int, formula_type: str = 'exponential') -> int:
        ...
        if formula_type == 'linear': return 100 * level
        elif formula_type == 'exponential': return 100 * (level ** 2)
        elif formula_type == 'logarithmic': return int(100 * level * math.log10(level + 1))
        return 100 * (level ** 2)
